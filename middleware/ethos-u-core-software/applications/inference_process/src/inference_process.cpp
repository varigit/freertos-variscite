...
#define ALIGN_SIZE(size) ((size + BUFFER_ALIGNMENT - 1) & (~(BUFFER_ALIGNMENT - 1)))
    ...
    int ret;
    ret = ethosu_invoke_v3(drv, custom_data_ptr, custom_data_size, base_addr,
...
bool InferenceProcess::runModel(InferenceJob &job) {
...
#if ETHOSU_LOG_SEVERITY >= ETHOSU_LOG_DEBUG
    tflite::NxpMicroErrorReporter errorReporter;
#else
    ...
    LOG_DEBUG("Operator(s) total: %" PRIu64 " CPU cycles", profiler.GetTotalTicks());
    LOG_DEBUG("Inference runtime: %" PRIu64 " CPU cycles total", job.cpuCycles);
