...
static int32_t isr_counter0 = 0; /* RL_PLATFORM_IMX95_M7_A55_USER_LINK_ID isr counter */
...
int32_t platform_init_interrupt(uint32_t vector_id, void *isr_data)
{
    ...
    {
        case RL_PLATFORM_IMX95_M7_A55_COM_ID:
            RL_ASSERT(0 <= isr_counter0);
            if (isr_counter0 == 0)
            {
                MU_EnableInterrupts(RPMSG_LITE_M7_A55_MU, (uint32_t)kMU_Rx0FullInterruptEnable << RPMSG_MU_CHANNEL);
            }
            isr_counter0++;
            break;
        default:
...
int32_t platform_deinit_interrupt(uint32_t vector_id)
{
    ...
    switch (RL_GET_COM_ID(vector_id))
    {
        case RL_PLATFORM_IMX95_M7_A55_COM_ID:
            ...
            if (isr_counter0 == 0)
            {
                MU_DisableInterrupts(RPMSG_LITE_M7_A55_MU, (uint32_t)kMU_Rx0FullInterruptEnable << RPMSG_MU_CHANNEL);
            }
            break;
        default:
            /* All the cases have been listed above, the default clause should not be reached. */
...
void platform_notify(uint32_t vector_id)
{
    ...
    {
        case RL_PLATFORM_IMX95_M7_A55_COM_ID:
            MU_SendMsg(RPMSG_LITE_M7_A55_MU, RPMSG_MU_CHANNEL, msg);
            break;
        default:
            /* All the cases have been listed above, the default clause should not be reached. */
            break;
...
int32_t MU7_B_IRQHandler(void)
{
    ...
    if ((((uint32_t)kMU_Rx0FullFlag << RPMSG_MU_CHANNEL) & MU_GetStatusFlags(RPMSG_LITE_M7_A55_MU)) != 0UL)
    {
        channel = MU_ReceiveMsgNonBlocking(RPMSG_LITE_M7_A55_MU, RPMSG_MU_CHANNEL); // Read message from RX register.
        env_isr((uint32_t)((channel >> 16) | (RL_PLATFORM_IMX95_M7_A55_COM_ID << 3)));
    }
...
int32_t platform_interrupt_disable(uint32_t vector_id)
{
    ...
    switch (RL_GET_COM_ID(vector_id))
    {
        case RL_PLATFORM_IMX95_M7_A55_COM_ID:
            ...
            if (disable_counter0 == 0)
            {
                NVIC_DisableIRQ(RPMSG_LITE_M7_A55_MU_IRQn);
            }
            disable_counter0++;
            break;
        default:
...
{
    MU_Deinit(RPMSG_LITE_M7_A55_MU);
