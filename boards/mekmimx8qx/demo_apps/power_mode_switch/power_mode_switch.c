...
void Flexcan_Set_Power(bool on)
{
    if (on)
    {
        ...
        RTN_ERR(sc_pm_set_resource_power_mode(ipc, SC_R_CAN_0, SC_PM_PW_MODE_ON) != SC_ERR_NONE);
...
static void APP_Resume(lpm_power_mode_t targetMode, bool result)
{
    ...
    RTN_ERR(sc_pm_set_resource_power_mode(ipc, SC_R_M4_0_I2C, SC_PM_PW_MODE_ON) != SC_ERR_NONE);
    ...
    RTN_ERR(sc_pm_set_resource_power_mode(ipc, SC_R_I2C_1, SC_PM_PW_MODE_ON) != SC_ERR_NONE);
