...
static void SRTM_SaiSdmaAdapter_InitSAI(srtm_sai_sdma_adapter_t handle, srtm_audio_dir_t dir)
{
    ...
    if (dir == SRTM_AudioDirTx)
    {
        ...
        if (rtm->format > kAUDIO_Stereo32Bits) /* DSD mode, need enable SDMA multi fifo */
        {
                                            ...
                                            uint32_t srate)
{
    ...
    srtm_sai_sdma_runtime_t rtm = dir == SRTM_AudioDirTx ? &handle->txRtm : &handle->rxRtm;
    uint32_t sai_clk;
    if (format > kAUDIO_Stereo32Bits && format <= kAUDIO_DSD32bits)
    {
    ...
    {
        if (format <= kAUDIO_Stereo32Bits) /* PCM mode */
        {
    ...
    if (rtm->srate != srate)
    {
        if (srate % 11025 == 0U)
        {
            ...
            sai_clk = CLOCK_GetPllFreq(kCLOCK_AudioPll2Ctrl) / (CLOCK_GetRootPreDivider(kCLOCK_RootSai1)) /
        ...
        else
        {
            ...
            sai_clk = CLOCK_GetPllFreq(kCLOCK_AudioPll1Ctrl) / (CLOCK_GetRootPreDivider(kCLOCK_RootSai1)) /
                      (CLOCK_GetRootPostDivider(kCLOCK_RootSai1));
        }
        if (dir == SRTM_AudioDirTx)
        {
            handle->txConfig.mclk = sai_clk;
            handle->txConfig.bclk = handle->txConfig.mclk;
        }
        else
        {
            handle->rxConfig.mclk = sai_clk;
...
static void SRTM_SaiSdmaAdapter_SetFormat(srtm_sai_sdma_adapter_t handle, srtm_audio_dir_t dir, bool sync)
{
    ...
    {
        cfg = &handle->txConfig;
        if (rtm->format > kAUDIO_Stereo32Bits) /* DSD mode */
        {
            xferFormat.channelMask = 1 << cfg->dataLine1 | 1 << cfg->dataLine2;
            SAI_TxSetDataOrder(handle->sai, kSAI_DataLSB); /* LSB transfer first */
        }
        else
        {
            xferFormat.channelMask = 1 << cfg->dataLine1;
            SAI_TxSetDataOrder(handle->sai, kSAI_DataMSB); /* MSB transfer first */
        }
...
static srtm_status_t SRTM_SaiSdmaAdapter_Start(srtm_sai_adapter_t adapter, srtm_audio_dir_t dir, uint8_t index)
{
    ...
    if (thisRtm->channels == kSAI_Stereo)
    {
        channelNum = 2U;
    }
    else
    ...
    srtm_sai_adapter_t adapter, srtm_audio_dir_t dir, uint8_t index, uint8_t format, uint8_t channels, uint32_t srate)
{
    ...
    if (format > kAUDIO_DSD32bits || channels >= ARRAY_SIZE(saiChannelMap))
    {
