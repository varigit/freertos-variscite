 ...
 * Definitions
 ******************************************************************************/
#define DEMO_LPADC_BASE         DMA__ADC1
#define DEMO_LPADC_USER_CHANNEL 2U
#define DEMO_LPADC_USER_CMDID   1U /* The available command number are 1-15 */

#define DEMO_LPADC_IRQn             DMA_ADC1_INT_IRQn
#define DEMO_LPADC_IRQ_HANDLER_FUNC DMA_ADC1_INT_IRQHandler
...
int main(void)
{
    ...
    if (sc_pm_set_resource_power_mode(ipc, SC_R_ADC_1, SC_PM_PW_MODE_ON) != SC_ERR_NONE)
    {
        PRINTF("Error: Failed to power on ADC\r\n");
    }

    IRQSTEER_Init(IRQSTEER);
    IRQSTEER_EnableInterrupt(IRQSTEER, DEMO_LPADC_IRQn);
