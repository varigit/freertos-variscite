...
- pin_list for VAR-SOM-MX8:
  - pin not set due to DEBUG_UART conflicts
...
#endif
  err = sc_pad_set_all(ipc, BOARD_INITPINS_M40_TPM0_CH0_PIN_FUNCTION_ID, 1U, SC_PAD_CONFIG_NORMAL, SC_PAD_ISO_OFF, 0x0 ,SC_PAD_WAKEUP_OFF);/* IOMUXD_M40_GPIO0_00 register modification value */
  if (SC_ERR_NONE != err)
  {
      assert(false);
  }
  err = sc_pad_set_all(ipc, BOARD_INITPINS_M40_TPM0_CH1_PIN_FUNCTION_ID, 1U, SC_PAD_CONFIG_NORMAL, SC_PAD_ISO_OFF, 0x0 ,SC_PAD_WAKEUP_OFF);/* IOMUXD_M40_GPIO0_01 register modification value */
  if (SC_ERR_NONE != err)
  {
      assert(false);
  }
