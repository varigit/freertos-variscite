...
static void SRTM_PdmEdmaAdapter_LocalBufFullProc(srtm_dispatcher_t dispatcher, void *param1, void *param2)
{
    ...
    EDMA_CreateHandle(&handle->extBufDmaHandle.wDmaHandle, handle->dma, rtm->extBuf.bufWriteDmaChannel);
    EDMA_SetCallback(&handle->extBufDmaHandle.wDmaHandle, SRTM_PdmEdmaAdapter_LocalBufFullDMACb, handle);
    EDMA_ResetChannel(handle->extBufDmaHandle.wDmaHandle.base, handle->extBufDmaHandle.wDmaHandle.channel);
    EDMA_InstallTCDMemory(&handle->extBufDmaHandle.wDmaHandle, tcdMemoryPoolPtr, TCD_QUEUE_SIZE);
    ...
    if (periods < periodsPerExt) /* The localbuf is a ringbuffer, there are more periods need to be copied. */
    {
        ...
        EDMA_PrepareTransfer(&transferConfig, (void *)src, sizeof(uint32_t), (void *)dst, sizeof(uint32_t),
                             sizeof(uint32_t), size, kEDMA_MemoryToMemory);
...
static void SRTM_PdmEdmaAdapter_DmaMemcpy(srtm_pdm_edma_adapter_t handle, uint32_t *dest, uint32_t *src, uint32_t count)
{
    ...
    EDMA_CreateHandle(&handle->extBufDmaHandle.rDmaHandle, handle->dma, rtm->extBuf.bufReadDmaChannel);
    ...
    EDMA_PrepareTransfer(&transferConfig, (void *)src, sizeof(dest[0]), (void *)dest, sizeof(dest[0]),
                         sizeof(src[0]), count, kEDMA_MemoryToMemory);
