...
#define APP_PANEL_FB_STRIDE_BYTE  (SDK_SIZEALIGN(APP_BPP * APP_PANEL_WIDTH, APP_WARP_COOR_BPP))
...
void APP_DPU_ArbitaryWarping(void)
{
    ...
    /*
     * Step 1: Configure the content stream
     *
...
void APP_DPU_Warp(void)
{
    ...
    /* Store unit. */
    DPU_InitStore(APP_DPU, kDPU_Store9, DPU_MAKE_SRC_REG1(kDPU_UnitSrcFetchRot9));

    DPU_DstBufferGetDefaultConfig(&dbConfig);
    dbConfig.baseAddr     = (uint32_t)s_displayFrameBuffer[1];
    ...
    warpConfig.srcReg           = DPU_MAKE_SRC_REG1(kDPU_UnitSrcFetchEco9);
    ...
    DPU_InitFetchUnitWarp(APP_DPU, kDPU_FetchRot9, &warpConfig);
    ...
    DPU_SetFetchUnitOffset(APP_DPU, kDPU_FetchRot9, 0, 0, 0);
    ...
    DPU_InitWarpCoordinates(APP_DPU, kDPU_FetchEco9, &coordinateConfig);
...
int main(void)
{
    ...
    APP_InitFrameBuffer();
    APP_DPU_Warp();
    memcpy(s_displayFrameBuffer[0], (void *)s_frameBuffer, sizeof(s_displayFrameBuffer[0]));
