...
/* FlexCAN */
#define APP_WAKEUP_CAN_NAME "FlexCAN1"
#define APP_WAKEUP_BUTTON_NAME "SW7 PWR ON"
#define APP_WAKEUP_PAD_NAME "UART RX Pad"
#define APP_WAKEUP_PAD SC_P_ADC_IN2

#define APP_CAN ADMA__CAN1
#define APP_CAN_CLK_FREQ CLOCK_GetIpFreq(kCLOCK_DMA_Can1)
#define APP_CAN_IRQn ADMA_FLEXCAN1_INT_IRQn
#define APP_CAN_IRQ_PRIO (4U)
#define DEMO_FORCE_CAN_SRC_OSC (1U)
...
void Flexcan_Set_Power(bool on)
{
    if (on)
    {
        ...
        RTN_ERR(sc_pm_set_resource_power_mode(ipc, SC_R_CAN_1, SC_PM_PW_MODE_ON) != SC_ERR_NONE);

        /* Set Peripheral clock frequency. */
        if (CLOCK_SetIpFreq(kCLOCK_DMA_Can1, SC_24MHZ) == 0)
        {
         ...
         */
        RTN_ERR(sc_pm_set_resource_power_mode(ipc, SC_R_CAN_1, SC_PM_PW_MODE_LP));
    }
    else
    {
        RTN_ERR(sc_pm_set_resource_power_mode(ipc, SC_R_CAN_1, SC_PM_PW_MODE_OFF));
...
static void APP_Resume(lpm_power_mode_t targetMode, bool result)
{
    ...
    if (kAPP_WakeupSourceCan == g_wakeupSource)
    {
        ...
        RTN_ERR(sc_pm_set_resource_power_mode(ipc, SC_R_CAN_1, SC_PM_PW_MODE_ON));
        if (CLOCK_SetIpFreq(kCLOCK_DMA_Can1, SC_24MHZ) == 0)
        {
            PRINTF("Error: Failed to set FLEXCAN frequency\r\n");
        }
        ...
        {
            RTN_ERR(sc_misc_set_control(ipc, SC_R_CAN_1, SC_C_IPG_STOP, 0U));
        }
    }
}
...
static void APP_SetWakeupConfig()
{
    ...
    else if (kAPP_WakeupSourceCan == g_wakeupSource)
    {
        ...
        RTN_ERR(sc_misc_set_control(ipc, SC_R_CAN_1, SC_C_IPG_STOP, 1U));
