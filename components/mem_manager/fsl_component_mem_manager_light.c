...
#if (defined __CORTEX_M) && ((__CORTEX_M == 4U) || (__CORTEX_M == 7U) || (__CORTEX_M == 33U))
    __asm("DSB"); /* __DSB() could not be used */
#endif            /* (defined __CORTEX_M) && ((__CORTEX_M == 4U) || (__CORTEX_M == 7U) || (__CORTEX_M == 33U)) */
...
mem_status_t MEM_BufferFree(void *buffer /* IN: Block of memory to free*/)
{
    ...
    else
    {
        ...
        /*
         * The workaround for IAR optimization issue. And code block might get removed after the optimization issue is
         * fixed by IAR.
         */
#ifdef __IAR_SYSTEMS_ICC__
#if (defined __CORTEX_M) && ((__CORTEX_M == 4U) || (__CORTEX_M == 7U) || (__CORTEX_M == 33U))
